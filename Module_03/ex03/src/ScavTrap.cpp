...
ScavTrap::ScavTrap(std::string name) : ClapTrap(name) {
	std::cout << MAGENTA "ScavTrap " RESET " name constructor is called!" << std::endl;
	_hit_point = 100;
	_energy_point = 50;
	_attack_damage = 20;
}
...
ScavTrap::ScavTrap(ScavTrap const & other): ClapTrap(other) {
	std::cout <<MAGENTA "ScavTrap " RESET "copy constructor is called!" << std::endl;
	// *this = other;
}
...
ScavTrap::~ScavTrap() {
	std::cout << MAGENTA "ScavTrap " GREEN << _name << RESET " destructor is called!" << std::endl;
}

void	ScavTrap::attack(std::string const & target){
	if(_energy_point && _hit_point){
		std::cout << MAGENTA "ScavTrap " GREEN << _name << RESET " attacks: " << RED << target << RESET " causing " RED << _attack_damage << RESET " points of damage." << std::endl;
		_energy_point--;
	}
	if (_hit_point == 0) 
		std::cout << MAGENTA "ScavTrap " GREEN << _name << RESET " is already dead!!!" << std::endl;
	if (_energy_point == 0)
