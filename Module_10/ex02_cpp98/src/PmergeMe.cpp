...
void PmergeMe::mergeInsertionSortVector(std::vector<int>& arr) {
	...
	int singleElement = -2; // Initialize with a value that won't appear in input
    if (n % 2 != 0) {
    ...
    // Sort pairs based on the larger element using the functor
    std::sort(pairs.begin(), pairs.end(), PairComparator());

    for (std::vector<std::pair<int, int> >::const_iterator it = pairs.begin(); it != pairs.end(); ++it) {
        if (it->second != -1) { // only add if it's not the marker
	...
	// If there was a single element, it needs to be added to pending
	if (singleElement != -2) {
		pending.push_back(singleElement); // Add the single element to pending
	}
    ...
    for (std::vector<int>::iterator it = pending.begin(); it != pending.end(); ++it) {
        insertElementVector(mainChain, *it, 0, mainChain.size() - 1);
    }
...
void PmergeMe::mergeInsertionSortList(std::list<int>& arr) {
    ...
    int singleElement = -2; // Initialize
    ...
    if (n % 2 != 0) {
        singleElement = *it;
        pairs.push_back(std::make_pair(*it, -1)); // Still use -1 as a marker
    }
    ...
    // If there was a single element, it needs to be added to pending
    if (singleElement != -2) {
        pending.push_back(singleElement);
    }

    // Insert pending elements into the main chain using linear insertion
    for (std::list<int>::iterator it = pending.begin(); it != pending.end(); ++it) {
        insertElementList(mainChain, mainChain.begin(), mainChain.end(), *it);
    }

    arr = mainChain;
}

void PmergeMe::insertElementList(std::list<int>& arr, std::list<int>::iterator left, std::list<int>::iterator right, int val) {
